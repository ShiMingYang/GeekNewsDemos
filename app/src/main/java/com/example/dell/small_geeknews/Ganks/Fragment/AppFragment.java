...
import com.example.dell.small_geeknews.Ganks.adapter.AppAdapter;
...
public class AppFragment extends Fragment {
    ...
    private ArrayList<AppBean.ResultsBean> list;
    private AppAdapter adapter;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View inflate = LayoutInflater.from(getContext()).inflate(R.layout.appall, null);
        initView(inflate);
        initData();
        return inflate;
    }
    ...
    private void initData() {
        ...
        Observable<AppBean> androiddata = mySerVer.appdata();

        androiddata.subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<AppBean>() {
                    @Override
    ...
    private void initView(View inflate) {
        ...
        Glide.with(getContext()).load(R.drawable.m9).apply(options).into(mIvTechBlur);
        Glide.with(getContext()).load(R.drawable.m9).into(mIvTechOrigin);
